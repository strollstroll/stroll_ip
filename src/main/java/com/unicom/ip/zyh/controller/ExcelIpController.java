...
import com.unicom.ip.zyh.service.ExcelIpService;
...
public class ExcelIpController {
  ...
  //获取excel批量导入后的数据  
	...
	public String importIpform(HttpSession session,HttpServletResponse response)  {
		List<ExcelIp> list=this.excelIpService.getAllExcelIp();
		if(null==list||list.size()==0) {
			return "预览表中没有待导入的数据！";
		}
		...
		if(flag!=0) {
			//添加到日志
		    operationLogService.addOperationLog(session, "excle批量导入部分失败", "批量导入成功"+(list.size()-flag)+"条，失败"+flag+"条");
			return "有"+flag+"条IP与IP列表中的IP地址重复！请刷新检查后再次提交。";
		}else {
			//添加到日志
		    operationLogService.addOperationLog(session, "excle批量导入", "批量导入"+list.size()+"条IP");
			return "导入IP地址表成功！";
		}
