...
public class UnconfirmIp {
	private Integer unconfirmId;

    public Integer getUnconfirmId() {
		return unconfirmId;
	}

	public void setUnconfirmId(Integer unconfirmId) {
   ...
   @JsonFormat(pattern="yyyy-MM-dd")
	...
	public String toString() {
		return "序号："+ipNumber+",状态：" + ipStatus + ",备注：" + ipRemarks + ",IP地址：" + ipAddress + ",子网掩码：" 
				+ ipSubnetmask + ",地址数量：" + ipAddressnumber + ",起用时间：" + ipUsetime + ",用户名称：" + ipUsername + ",vlan号：" 
				+ ipVlan + ",连接设备：" + ipConnectingdevice + ",端口：" + ipPort + ",速率：" + ipRate + ",运维系统编号：" + ipAttribution +
				",宽带受理编号：" + ipBroadbandacceptancenumber + ",SN号=：" + ipSnnumber + ",OLT地址：" + ipOltaddress + ",IOM用户：" + ipIomusername + ",装机地址：" 
				+ ipInstalledaddress + ",类型：" + ipType + ",WOTV-BSS主产品(备注2)：" + ipWotvbssremarks + 
				",上行速率：" + ipOutputrate + ",下行速率：" + ipInputrate + ",对应终端数：" + ipTerminalnumber;
	}
